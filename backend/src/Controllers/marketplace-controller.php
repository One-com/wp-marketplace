 ...
 *
 * @package Groupone\Marketplace\Controllers
 */
 ...
 *
 * Handles the marketplace logic, including menu registration, REST API endpoints, and AJAX actions.
 */
class MarketplaceController {
	/**
	 * Configuration array.
	...
	/**
	 * Marketplace model instance.
	 *
	 * @var MarketplaceModel|null
	 */
	protected $model;
	...
	protected function ensure_assets_resolved() {
		if ( null === $this->assets_base_path || null === $this->assets_base_url ) {
	...
	protected function find_package_root_via_composer() {
		...
		for ( $i = 0; $i < $max_depth; $i++ ) {
			...
			if ( file_exists( $composer_path ) ) {
				...
				if ( isset( $composer_data['name'] ) && 'groupone/marketplace' === $composer_data['name'] ) {
					return $current_dir;
				}
			}
	...
	public function register_addons_menu() {
		$menu_slug        = $this->config['addons_menu_slug'] ? $this->config['addons_menu_slug'] : 'onecom-marketplace-products';
		$page_title       = $this->config['addons_page_title'] ? $this->config['addons_page_title'] : __( 'Marketplace Products', 'onecom-wp' );
		$menu_title       = $this->config['addons_menu_title'] ? $this->config['addons_menu_title'] : __( 'Your add-ons', 'onecom-wp' );
	...
	public function render_addons_page() {
		...
		$localized_config = [
			...
			'wpConfig'            => [
				...
				'rankMathRegistrationSkip' => (bool) ( ! empty( get_option( 'rank_math_registration_skip' ) ) && ( '1' === get_option( 'rank_math_registration_skip' ) || true === get_option( 'rank_math_registration_skip' ) ) ),
			],
	...
	/**
	 * Get plugins from marketplace catalog.
	 *
	...
	public function ajax_install_plugin() {
		...
		$download_url = isset( $_REQUEST['download_url'] ) ? esc_url_raw( wp_unslash( $_REQUEST['download_url'] ) ) : '';
	...
	private function resolve_plugin_file_by_slug( $slug ): string {
		...
		if ( false !== strpos( $slug, '/' ) || '.php' === substr( $slug, -4 ) ) {
	 ...
	 * @param array        $hook_extra Array of additional hook data.
	 * @return void
	 */
