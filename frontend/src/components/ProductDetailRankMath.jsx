...
export default function ProductDetailRankMath({
  ...
  if (i18nSource && typeof i18nSource === 'object') {
    ...
    while (i18nSource[`coreFeatureTitle${i}`] || i18nSource[`coreFeatureContent${i}`]) {
      const featureTitle = i18nSource[`coreFeatureTitle${i}`];
      const featureContent = i18nSource[`coreFeatureContent${i}`];
      if (
        featureTitle &&
        featureTitle.trim() !== '' &&
        featureContent &&
        ...
        coreFeaturesFromI18n.push({ name: featureTitle, desc: featureContent });
      }
      i++;
    }
  }
