...
export default function ProductDetailRankMath({
  ...
  useEffect(() => {
    ...
    const navIsAtBottom = false;
    const paginationState = 'top'; // 'top', 'overlay', or 'bottom'
    const isPaginationScrolledPast = false;
    const isTableBottomVisible = false;
    const isUpdatingClasses = false; // Flag to prevent observer callbacks during class updates

    // Throttle timer for scroll events
    const scrollThrottleTimer = null;
    const isScrollThrottled = false;
  ...
  useEffect(() => {
    ...
    tableBottomSentinel.style.width = '100%';
    tableBottomSentinel.style.height = '1px';
    tableBottomSentinel.style.visibility = 'hidden';
    ...
    const tableBottomObserver = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          // Check if table bottom sentinel is in view
    ...
    // Window scroll and resize events for nav position and boundary check
    const handleScrollAndResize = () => {
      ...
      // Recalculate nav position and boundary
      const { bottomValue: currentBottomValue } = calculateNavPosition();
      checkNavBoundary(currentBottomValue);
    ...
    return () => {
      ...
      if (table.contains(tableBottomSentinel)) {
        table.removeChild(tableBottomSentinel);
      }
